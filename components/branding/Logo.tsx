...
export function Logo({ variant = "full", className = "" }: LogoProps) {
  ...
  if (variant === "compact") {
    return (
      <Link href={homeHref} className={`relative top-[3px] inline-flex items-center gap-2.5 ${className}`} aria-label={`${brandName} Shenzhen 2026 home`}>
  ...
  return (
    <Link href={homeHref} className={`inline-flex flex-col ${className}`} aria-label={`${brandName} Shenzhen 2026 home`}>
      <span className="editorial-type text-2xl tracking-[-0.015em]">{brandName}</span>
